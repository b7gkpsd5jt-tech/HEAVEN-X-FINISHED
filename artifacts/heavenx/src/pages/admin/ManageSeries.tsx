...
export default function ManageSeries() {
  ...
  return (
    <div className="p-6 text-white">
      ...
      {series.map(s => <div key={s.id} className="p-4 border-b">{s.title}</div>)}
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4">
          <form onSubmit={handleSubmit} className="bg-[#111] p-6 rounded-xl w-full max-w-md">
            ...
            <button type="button" onClick={() => setShowModal(false)} className="ml-2 bg-gray-600 px-4 py-2 rounded">Abbrechen</button>
          </form>
        </div>
      )}
    </div>
